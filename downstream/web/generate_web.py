...
class WritableDirectory(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if os.access(values, os.R_OK + os.W_OK):
            ...
            raise argparse.ArgumentTypeError("{0} is not a writable directory"
                                             "".format(values))
...
def process_record(output, record):
    """
    Creates corresponding html with name and content and returns <li> element
    for landing page
    """
...
{name:                 '$NAME',
bwgURI:               '/$FILE',
style: [{type: 'default', style: {glyph: 'HISTOGRAM', BGCOLOR: 'rgb($RGB)', \
HEIGHT: 30, id: 'style1'}}],
noDownsample:         true},
""".replace('$NAME', os.path.basename(f)).replace('$FILE', f).replace('$RGB',
                                                                      rgb))
        ...
        with open(file, 'w') as file:
            file.write(data_html.
                       ...
                       replace('//$DATA_HERE', '\n'.join(tracks)))
        print('Created', file)
        return '<li><a href="{0}/{1}.html">{1}</a></li>'.format(output,
                                                                record.name)
...
def process(output, records):
    ...
    browsers = '\n'.join(filter(lambda x: x is not None,
                                [process_record(output, r) for r in records]))
...
generate_web.py is a script to generate index.html and other html files for \
ageing web server.

USAGE: generate_web.py --output out --browsers name1 type1 folders11,...,\
folder1n1 name2 type2 folder21,..,folder2n2
Supported types: bw and dir.

EXAMPLE:
python ~/work/washu/web/generate_web.py --output . --browsers k27ac bw \
...
''')
    parser.add_argument('--output', required=True, action=WritableDirectory,
                        type=str,
                        help='Path to directory with data to run pipeline')
    parser.add_argument('--browsers', required=True,
                        metavar='name browser_type dirs', type=str, nargs='+',
